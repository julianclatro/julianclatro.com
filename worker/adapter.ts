...
// import { createRequestHandler as createRemixRequestHandler } from '@remix-run/cloudflare-pages'
...
export function createFetchHandler<Env>({
  ...
  return async (request: Request, env: Env, ctx: ExecutionContext) => {
    try {
      ...
      if (isHeadOrGetRequest && cache) {
        const isCached = cache?.put(request, response.clone())
        ctx.waitUntil(isCached)
      }
...
function newRequest(request: Request, headerFn: any): Request {
